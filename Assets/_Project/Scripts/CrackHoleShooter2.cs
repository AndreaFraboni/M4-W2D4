...
public class CrackHoleShooter2 : MonoBehaviour
{
        ...
        if (Input.GetMouseButtonDown(0))
        {
            ...
            if (Physics.Raycast(start, direction, out RaycastHit hit, _maxDistance, _targetLayer))
            {
                if (hit.collider.attachedRigidbody != null)
                {
                    ...
                    // converto punto di HIT in coordinate locali sulla faccia del cubo
                    Vector3 _localHitPoint = hit.transform.InverseTransformPoint(hit.point);
                    Debug.Log("Local hit: " + _localHitPoint);

                    // Prendiamo riferimenti alle dimensioni del quad con il crack hole
                    Renderer _quadRenderer = _crackHolePrefab.GetComponent<Renderer>();
                    float _quadSizeX = _quadRenderer.bounds.size.x;
                    float _quadSizeY = _quadRenderer.bounds.size.y;
